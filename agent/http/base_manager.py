...
class BaseManager(object):
    """
    ### Ipython Example:
    In [1]: from cloud_admin.manager.base_manager import BaseManager
    In [2]: bm = BaseManager()
    In [3]: t = bm.app.test_client()
    ## PUT example:
    In [4]: resp = t.put('/execute', data='{"command": "ls -la", "verbose": true, "timeout": 10}')
    In [5]: print resp.get_data()
    ## GET example:
    In [6]: print t.get('/status').get_data()
    ...
    def test_client(self,use_cookies=True, **kwargs):
        return self.app.test_client(use_cookies=use_cookies, **kwargs)
